...
- **公国版图 1-10**:高清源图已定位(Gamefound 更新 API),逐格勘定待后续。
...
### P0:版图 1-10 逐格勘定(高优先)
- **现状**:boards.json 全部占位(`cells: []`, `confidence: "missing"`),引擎使用兜底布局(37 格手工摆一张可玩布局)。
- **物料**:D2 已定位 Gamefound 高清源图(`_cob/d2/gf/`),2560px+ 原图,编号 1-10 已确认。
- **方法**:主代理直接目读 `gf/u20_2.png` 等图,按行提取 37 格(颜色+骰点),写入 boards.json。
- **验收**:每张版图通过 check.mjs(37 格、无重复坐标、颜色合法)。
